...
`isaac_lab_env/open_duck_mini_v2/amp/`. Reference clips generated by
`scripts/convert_gait_library_to_amp.py` (50 fps, velocities differentiated
from the CLAMPED trajectory — see journal run 4 for why). Train via
`scripts/train_amp.py --algorithm AMP`; gate-check with
`scripts/measure_amp_gait.py`; run history in the experiment journal.
