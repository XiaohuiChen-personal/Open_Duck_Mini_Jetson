...
## Reward Functions (v4 — Imitation Reward)

11 reward terms: 4 positive + 7 negative. No alive bonus.

The imitation reward is the **dominant positive signal** — it tracks reference leg joint positions from the Open Duck Playground polynomial gait library (240 motions, degree-15 polynomials, 0.54s period).

**Positive rewards:**
- `imitation_reward` (ImitationReward class): weight=**10.0** — exp(-2 * squared_error) over 10 leg joints vs polynomial reference. Matches closest velocity command.
- `track_lin_vel_xy_yaw_frame_exp`: Velocity tracking (std=0.25, weight=2.0)
- `track_ang_vel_z_world_exp`: Yaw tracking (std=0.5, weight=1.0)
- `feet_air_time_positive_biped`: Biped gait (weight=0.25, threshold=0.2, body_names=foot_assembly/foot_assembly_2)

**Penalties:**
- `is_terminated`: Fall penalty (-200)
- `base_height_l2`: Height control at 0.20m (-2.0)
- `flat_orientation_l2`: Stay upright (-1.0)
- `lin_vel_z_l2`: Vertical bouncing (-1.0)
...
**Reward evolution:**
- v1: H1-derived, 13 penalties → structurally negative, failed
- v2: Added alive bonus → crouching/shuffling exploit, failed
- v3: No alive bonus, height control → improved but unnatural gait
- v4: Imitation reward + phase observation → natural bipedal gait, mean reward 147 at 100 iterations
